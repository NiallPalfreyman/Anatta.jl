...
[
    Activity(
        """
        Hi! Welcome to Anatta Lab 601: How do we analyse and visualise ABM results?
    ...
    Activity(
        ...
        Find the learning activity "tribe position" in demo(). The vector `adata` in the next line
    ...
    Activity(
        """
        Things are looking good, but mean tribal x-position is only a very approximate indicator of
        ...
        Append GLMakie to the using statement under the learning activity "graphics backend", then
        insert the following code under the learning activity "generate video":
    ...
    Activity(
        ...
        mean tribal x-position were only very small? The video shows us very clearly that
        segregation is occurring in our simulation, but the segregated regions of the two tribes
        are so closely intertwined that their mean x-positions might easily be almost equal.
        ...
        possibility. Insert the following lines of code under the learning activity "create an
    ...
    Activity(
        """
        Look carefully at the code you have just entered. The second return value of
        abmexploration() is not of interest to us, but the first return value (playground) is an
        application that we use to explore the Schelling model visually. The first argument calls
        schelling() to create a new Schelling model, and the `params` Dictionary specifies the
        Independent Variables of our experiment. In our case, we want to vary independently the
        value of preference between 0.0 and 1.0 (in steps of 0.1), and explore how this affects
    ...
    Activity(
        ...
        agents that are segregated. Under the learning activity counting "tribally similar
    ...
    Activity(
        ...
        level of 1/20! Now, 30% is not very extreme; it corresponds to black, white or asian
        citizens preferring just 3 out of every 10 of their neighbours to come from the same ethnic
        background. It corresponds to women preferring just 3 out of every 10 people at their
        workplace to be female. Yet this preference (not requirement!) drives communities to a
        point where 1 in 20 citizens spend their lives in a social or work context Entirely
        surrounded by citizens of similar skin-colour, ethnicity or gender.
