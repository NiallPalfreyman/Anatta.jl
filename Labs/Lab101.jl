...
[
    ...
    Activity(
        ...
        method evaluate() in the module Semantics, which returns the truth value of a given wff
        ...
        evaluate() method. Implement and test your code with each of these possible wffs, and if
        it produces correct output for ALL of them, you can be satisfied that your work is
        reasonably correct.

        In particular, make sure that demo()'s explicit call to the method evaluate() generates
        correct output. To do this, you'll need to experiment with various models and wffs, but make
        sure you restore both model and evaluated wff to their original values after experimenting.
        Then reply() me ...
        """,
        "",
        x -> begin
    ...
    Activity(
        ...
        while Semantics are generated by the truth-value rules specified in Semantics.evaluate().
        But do wffs and truth-tables describe the same beliefs? Do syntactic structures trule
        cover the EXACTLY same ground as their dynamical meaning?
    ...
    Activity(
        """
        These questions concern the Soundness and Completeness of the relationship between
        syntactic structures and their semantic meaning:
        ...
        In fact, YOU have already proven that the PL-truth-table relationship is sound! Which method
        did you implement that generates truth-table semantics from a PL proposition?
        """,
    ...
    Activity(
        """
        To complete this lab, we will prove computationally that PL is Complete with respect to
    ...
    Activity(
        """
        Congratulations! You have now proven the Soundness and Completeness of the language PL
        of propositional logic! :))

        Our next lab will take us to the Next Level: Syntactic Structure and Incompleteness!
        """,
        "",
        x -> true
    ),
